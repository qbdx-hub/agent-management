...
CREATE TABLE IF NOT EXISTS `workspace` (
  ...
  `status` TINYINT NOT NULL DEFAULT 1 COMMENT '状态: 0-禁用 1-启用',
  -- Agent 执行环境策略（2026-09-04 V12，对标 Claude Code 项目级权限）
  `shared_workdir` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '共享工作目录：0-每会话独立沙箱(ws-{id}/session-{id})，1-空间内会话共享文件区(ws-{id})',
  `allow_outside_sandbox` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '允许沙箱外运行总闸：0-空间内禁止授权逃逸沙箱，1-允许（成员才可在会话中开启沙箱外运行）',
  `disabled_tools` VARCHAR(500) DEFAULT NULL COMMENT '空间级禁用的内置工具名，逗号分隔（如 run_command,web_fetch），NULL/空=全部允许',
  -- 成员终端开关（2026-09-05 V13）
  `member_terminal_enabled` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '成员终端开关：0-移动端终端仅空间 owner/admin 可用，1-空间成员均可用（默认放开）',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `knowledge_base` (
  ...
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_kb_name` (`workspace_id`, `name`),
  KEY `idx_workspace_id` (`workspace_id`)
