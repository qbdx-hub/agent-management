...
- **2026-09-03**: 图标体系整体替换为 Tabler（unplugin-icons 编译期按需打包，离线无 CDN）：emoji PNG 头像/工具图标（/icons/*.png）与 Element Plus 填充图标全部下线——新增 `components/icons.ts` 注册表（约 70 枚 2px 线性图标）+ 全局 `<UiIcon>` 组件，Agent 头像/工具图标改为「语义图标 + 柔和底色板」（6 色稳定散列，旧 PNG stem 与 emoji 存量数据自动映射兼容，AgentCreate 头像选择器同步换新集）；侧边栏/顶栏/53 处内联图标/状态与趋势箭头/空态插图统一换 Tabler，移除 EP 全局图标注册，⚠️ 等文案 emoji 改纯文本标记。vue-tsc + vite build 通过（图标 chunk 6KB gzip）
- **2026-09-03**: 全站 UI 重构为「明亮产品风」设计系统（V4 定稿，基线 docs/design-previews/workbench-v4-bright.html）：新增 `styles/tokens.css` 设计令牌（唯一强调色 #5a54e8、一族冷灰、统一圆角 16/10/999 与扁平阴影系统），并将 Element Plus 主题变量整体改绑——全站组件（按钮/输入框/表格/菜单/标签/分页…）自动跟随，旧 EP 蓝 #409EFF 全库清零；工作台重排为四联指标瓦片 + Agent 卡直铺 + 右栏动态/快捷入口（消除右上死区与卡片套卡片）；列表页（Agent 管理/工具市场/编排/知识库/会话历史/审计/审批/成员/预算/告警）统一为「工具栏并入表格卡 + h2 20px/800 + 圆角标签 + 等宽数字」；详情页（Agent/工具/知识库/链路）拆掉外层卡片改为标签页直铺，消灭嵌套卡片与左右不对齐；成功率口径全链路统一为百分数 0-100 且无数据返回 NULL（后端 Agent/Tool/Monitor 三处 service 修正 + 前端 formatPercent 接百分数，修复 0 会话显示 10000.0% 的脏数据，V11 迁移清理历史行并把列默认值改为 NULL）；监控六指标卡固定一行窄屏 3/2 列降级，响应式 1200/820 断点。vue-tsc + vite build 通过
- **2026-09-03**: 聊天框新增「沙箱外运行」用户授权开关（会话控制台输入栏，默认关闭）：开启后本次消息的内置文件/命令工具从会话沙箱切换到服务器真实文件系统——相对路径以服务器进程目录（backend/）为基准、绝对路径放行、命令工作目录同为进程目录，并向模型注入执行环境授权 system 提示（含谨慎操作提醒）；SSE tool_call 事件携带 `outsideSandbox` 标志，前端步骤展示「沙箱外」警示标；超时/输出上限、路径安全校验、tool_call_record 归因均保持不变。已 E2E 验证：未授权读真实文件被沙箱拦截，授权后成功读取真实 README 并 `cd` 到 backend 目录
