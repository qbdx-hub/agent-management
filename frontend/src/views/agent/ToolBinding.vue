...
<template>
  <div class="tool-binding" v-if="agent">
    <div v-if="allTools.length > 0" class="binding-header">
      <span>选择要绑定到此 Agent 的工具（已选 <span class="num">{{ boundToolIds.length }}</span> 个）</span>
    ...
    <el-row :gutter="16">
      <el-col v-for="tool in allTools" :key="tool.id" :span="8">
        <el-card class="tool-bind-card" :class="{ bound: isBound(tool.id) }" @click="toggleTool(tool.id)">
          <div class="tool-bind-header">
            ...
            <UiIcon name="circle-check" :size="19" v-if="isBound(tool.id)" class="bind-check" style="color: #178a5b" />
          </div>
